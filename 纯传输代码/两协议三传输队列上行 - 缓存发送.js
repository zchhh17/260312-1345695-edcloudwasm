...
const manualPipe = async (readable, writable, close) => {
    ...
    const flushBuffer = () => {
        ...
        offset > 0 && (offset > safeBufferSize ? (writable.send(new Uint8Array(buffer, 0, offset)), buffer = new ArrayBuffer(bufferSize)) : writable.send(new Uint8Array(buffer.slice(0, offset))), offset = 0);
    ...
    try {
        while (true) {
            ...
            if (needsFlush) {
                ...
                globalCount[idx]++, globalBytes[idx] += chunkLen, statCount++, totalCount++, totalGlobalBytes += chunkLen, protectFlush = chunkLen < maxChunkLen;
                if (statCount > 16384) {
