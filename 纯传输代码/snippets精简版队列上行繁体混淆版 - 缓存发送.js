...
const 手動資料管線 = async (可讀流, 可寫通道, 關閉連線) => {
    ...
    let 偏移量 = 0, 總位元組 = 0, 時間 = 1, 計時器識別 = null, 恢復函式 = null, 正在讀取 = false, 需要刷新 = false, 保護刷新 = false;
        ...
        while (true) {
            ...
            if (需要刷新) {
                ...
                if (快速刷新) {
                    ...
                    const 索引 = 區塊長度 >= 30720 ? 13 : 區塊索引查找表[區塊長度 >> 9];
                    全域計數[索引]++, 全域位元組[索引] += 區塊長度, 統計計數++, 總計數++, 全域總位元組 += 區塊長度;
                    if (統計計數 > 16384) {
                        統計計數 = 0, 總計數 >>>= 1, 全域總位元組 >>>= 1;
                        for (let 索引 = 0; 索引 < 14; 索引++) 全域計數[索引] >>>= 1, 全域位元組[索引] >>>= 1;
                    }
                    let 最大分數 = -1, 最大索引 = 0;
                    const 位元組因子 = 0.25 * 總計數 / 全域總位元組;
                    for (let 索引 = 0; 索引 < 14; 索引++) {
                        const 分數 = 全域計數[索引] + 全域位元組[索引] * 位元組因子;
                        分數 > 最大分數 && (最大分數 = 分數, 最大索引 = 索引);
                    }
                    if (區塊長度 < 下限陣列[最大索引]) {
                        總位元組 = 0, 時間 = 1;
